...
- [ ] Write unit tests T-01, T-02, T-03 (escape helper) — all fail
- [ ] Write integration tests I-01 through I-11 — all fail
- [ ] Write E2E tests E-01, E-02, E-03 — all fail
- [ ] Create `lib/chat_app/search/fts5_index.ex`
- [ ] Implement `escape_fts_query/1` by delegating to `QueryProcessor.escape_fts_query/1`
- [ ] Run escape unit tests — green
- [ ] Implement `upsert/1`:
  - [ ] `Repo.get(Item, item_id)` — return `:ok` immediately if nil (graceful no-op)
  - [ ] Wrap both SQL statements in a `Repo.transaction/1`:
    - [ ] `INSERT INTO clothing_fts(clothing_fts, rowid, title) VALUES ('delete', ?, ?)` with current title
    - [ ] `INSERT INTO clothing_fts(rowid, title) VALUES (?, ?)` with new title
  - [ ] Return `:ok`
- [ ] Implement `search/2`:
  - [ ] Guard: return `[]` immediately if `query_text` is empty string
  - [ ] Do NOT call `escape_fts_query/1` — the caller is responsible (see no-double-escaping contract above)
  - [ ] Execute `SELECT rowid, bm25(clothing_fts) AS score FROM clothing_fts WHERE clothing_fts MATCH ? ORDER BY score ASC LIMIT ?`
  - [ ] Return `[{rowid, score}]` as `[{integer(), float()}]`
- [ ] Run all integration and E2E tests — all green

---

## Definition of Done

- [ ] All 17 tests green (`mix test test/integration/search/fts5_index_test.exs`)
- [ ] BM25 sort polarity explicitly verified: I-04 green with score negative and item 1 before item 2
- [ ] Upsert idempotency confirmed: I-02 green with exactly one result after two upserts for the same item
- [ ] `search("")` returns `[]` and does not raise (I-08 green)
- [ ] `upsert/1` for non-existent item returns `:ok` (I-03 green)
- [ ] External content table delete-then-insert pattern used (not INSERT OR REPLACE): verified by I-02 returning single result
- [ ] `search/2` does not call `escape_fts_query/1` internally — no double-escape regression
