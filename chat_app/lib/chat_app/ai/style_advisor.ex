defmodule ChatApp.AI.StyleAdvisor do
  ...
  @base_system_prompt "You are Threadworks AI, a style consultant specialising in second-hand clothing."
  ...
  def build_prompt(base, items) do
    item_lines =
      ...
      |> Enum.map_join("\n", fn {item, n} ->
        ...
        "[#{n}] #{item.title} | Size: #{item.size} | Condition: #{condition} | $#{price} | #{source}"
      end)
